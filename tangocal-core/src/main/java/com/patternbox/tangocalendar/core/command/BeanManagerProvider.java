...
/**
 * Original source: <b>org.apache.deltaspike.core.api.provider.BeanManagerProvider</b>
 * <p>
 * This class provides access to the {@link BeanManager} by registering the current
 * {@link BeanManager} in an extension and making it available via a singleton factory for the
 * current application.
 * </p>
 * <p>
 * This is really handy if you like to access CDI functionality from places where no injection is
 * available.
 * </p>
 * <p>
 ...
 * 
 * <p>
 * <b>Attention:</b> This method is intended for being used in user code at runtime. If this method
 * gets used during Container boot (in an Extension), non-portable behaviour results. During
 * bootstrapping an Extension shall &#064;Inject BeanManager to get access to the underlying
 * BeanManager (see e.g. {@link #cleanupFinalBeanManagers} ). This is the only way to guarantee to
 * get the right BeanManager in more complex Container scenarios.
 * </p>
 */
public class BeanManagerProvider implements Extension {
	...
	/**
	 * This data container is used for storing the BeanManager for each WebApplication. This is needed
	 * in EAR or other multi-webapp scenarios if the DeltaSpike classes (jars) are provided in a
	 * shared ClassLoader.
	 */
	private static class BeanManagerInfo {
		...
		/**
		 * The final BeanManagers. After the container did finally boot, we first try to resolve them
		 * from JNDI, and only if we don't find any BM there we take the ones picked up at startup.
		 */
		private BeanManager finalBm = null;

		/**
		 * Whether the CDI Application has finally booted. Please note that this is only a nearby value
	 ...
	 * </p>
	 * <p>
	 * <b>Attention:</b> This instance must only be used through the {@link #bmpSingleton} singleton!
	 * </p>
	 */
	private volatile Map<ClassLoader, BeanManagerInfo> bmInfos = new ConcurrentHashMap<ClassLoader, BeanManagerInfo>();

	/**
	 * Detect the ClassLoader.
	 * 
	...
	/**
	 * Returns if the {@link BeanManagerProvider} has been initialized. Usually it isn't needed to
	 * call this method in application code. It's e.g. useful for other frameworks to check if
	 * DeltaSpike and the CDI container in general have been started.
	 * 
	 ...
	 *           if the {@link BeanManagerProvider} isn't ready to be used. That's the case if the
	 ...
	 * {@link AfterBeanDiscovery} event since it allows to use the {@link BeanManagerProvider} for all
	 * events which occur after the {@link AfterBeanDiscovery} event.
	 * 
	...
	public void setBeanManager(@Observes AfterBeanDiscovery afterBeanDiscovery,
		...
		BeanManagerInfo bmi = getBeanManagerInfo(getClassLoader());
		bmi.loadTimeBm = beanManager;
	}

	/**
	 * The active {@link BeanManager} for the current application (/{@link ClassLoader}). This method
	 * will throw an {@link IllegalStateException} if the BeanManager cannot be found.
	 * 
		...
		if (result == null) {
			synchronized (this) {
				...
				if (result == null) {
					...
					// store the resolved BeanManager in the result cache until #cleanupFinalBeanManagers gets
					// called
	...
	/**
	 * By cleaning the final BeanManager map after the Deployment got Validated, we prevent premature
	 * loading of information from JNDI in cases where the container might not be fully setup yet.
	 * 
	 * This might happen if someone uses the BeanManagerProvider during Extension startup.
	 */
	public void cleanupFinalBeanManagers(@Observes AfterDeploymentValidation adv) {
		for (BeanManagerInfo bmi : bmpSingleton.bmInfos.values()) {
			...
			bmi.booted = true;
			/*
			 * possible issue with >weld< based servers: if #getBeanManager gets called in a custom
			 * AfterDeploymentValidation observer >after< this observer, the wrong bean-manager might get
			 * stored (not deterministic due to the unspecified order of observers). finally a
			 * bean-manager for a single bda will be stored and returned (which isn't the bm exposed via
			 * jndi).
			 */
		}
	}
	...
	public void cleanupStoredBeanManagerOnShutdown(@Observes BeforeShutdown beforeShutdown) {
		bmpSingleton.bmInfos.remove(getClassLoader());
	}
	...
	/**
	 * This function exists to prevent findbugs to complain about setting a static member from a
	 * non-static function. <br>
	 * Furthermore we can mock the bean manager provider for testing.
	 * 
	 * @param beanManagerProvider
	 *          the bean-manager-provider which should be used if there isn't an existing provider
	 * @return the first BeanManagerProvider
	 */
	public static BeanManagerProvider setBeanManagerProvider(BeanManagerProvider beanManagerProvider) {
		if (bmpSingleton == null) {
